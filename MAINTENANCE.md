...
2. 检查维护者、许可证、最近提交、Release、Issue 和实际文件。
3. 检查脚本、依赖、外部下载、网络调用、凭据读取和高权限命令。
4. 判断 Pi/OpenCode 属于原生、标准或需要适配。
5. 写明它解决的具体工作问题，并与现有条目去重。
6. 新且证据不足的项目先放入 `catalog/watchlist.md`。
7. 运行 `python scripts/validate_catalog.py`。
8. 只有发现有意义变化时才提交；纯排名波动不更新。
