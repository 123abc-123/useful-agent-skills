...
榜单来源和各自统计口径见 `content/catalog/discovery-sources.md`。每日任务至少交叉检查一个原始榜单、一个二次趋势来源和候选 Skill 的 GitHub 原仓库。
...
7. 新且证据不足的项目先放入 `content/catalog/watchlist.md`。
...
- `content/catalog/recommended-stack.md` 中的用途、命令和风险说明。
...
`content/data/prompts.json` 是原创 Prompt 的机器可读索引。新增或修改模板时必须：

- 使用 `content/prompts/<name>.prompt.md`，并包含 `name`、`description`、`category` frontmatter；
- 让索引中的变量与模板内全部 `{{variable}}` 精确一致；
- 写明输入、输出结构、证据要求和信息不足时的行为；
- 添加至少一个应满足和一个不应发生的回归案例；
- 不复制许可证不明的第三方 Prompt，不写入密钥、客户数据或内部路径；
- 运行 `python tooling/scripts/validate_catalog.py`。
...
```text
content/data/tools/<category>.json           # 人工维护
content/catalog/tools/<category>/README.md   # 自动生成
site/tools/<category>/index.html             # 自动生成
```

新增或更新工具子栏目时必须：

- 修改 JSON 后运行 `python tooling/scripts/build_catalog.py`，保持分类说明、结构化数据和网页卡片一致；
...
每周一可以生成一次 `content/reports/weekly/` 简报，汇总新增、上升、降级、移除和 Pi/OpenCode 兼容性变化。没有有意义变化时不创建空报告。
