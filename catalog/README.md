...
`catalog/` 保存人工整理的分类说明、推荐理由和采用边界。可安装 Skill 仍放在仓库根目录的 `skills/<skill-name>/`，分类目录不会改变 Pi/OpenCode 的发现路径。
...
第三方 Skill 只在 `catalog/` 和 `data/` 中记录，不把其源码复制进本仓库。需要试用时，使用固定来源安装到项目的 `.agents/skills/<skill-name>/`。
