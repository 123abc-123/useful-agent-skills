"""Generate Tools catalog Markdown and GitHub Pages from data/tools/*.json."""
...
def render_category_markdown(category: dict, payload: dict) -> str:
    ...
    return "\n".join(
        [
            f"# {category['title_zh']}",
            "",
            f"{category['description']} 机器可读数据见 [`data/tools/{category['slug']}.json`](../../../data/tools/{category['slug']}.json)，网页入口见 [{category['title']}](https://123abc-123.github.io/useful-agent-skills/tools/{category['slug']}/)。",
            "",
...
<body>
  ...
  <main>
    ...
    <footer>数据来自 <a href="https://github.com/123abc-123/useful-agent-skills/blob/main/data/tools/@@SLUG@@.json">data/tools/@@SLUG@@.json</a>。安装前请检查固定源码、依赖、权限和运行状态。</footer>
  </main>
  <script>
...
def main() -> int:
    ...
    write_or_check(ROOT / "catalog" / "tools" / "README.md", render_catalog_index(index, payloads), args.check, stale)
    write_or_check(ROOT / "docs" / "tools" / "index.html", render_tools_page(index, payloads), args.check, stale)
    for category in index["categories"]:
        slug = category["slug"]
        write_or_check(ROOT / "catalog" / "tools" / slug / "README.md", render_category_markdown(category, payloads[slug]), args.check, stale)
        write_or_check(ROOT / "docs" / "tools" / slug / "index.html", render_category_page(category), args.check, stale)

    if stale:
        print("Generated catalog files are stale:")
        for path in stale:
            print(f"- {path}")
        print("Run: python scripts/build_catalog.py")
        return 1
